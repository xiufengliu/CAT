...
def vars_for_all_templates(self):
    phase_no = self.session.config['app_sequence'].index('communication')
...
class GroupChoice(Page):
    ...
    def vars_for_template(self):
        coo_treatment_risks = self.session.vars['communication_risks'][self.round_number-1]
        risks = [coo_treatment_risks[self.player.id_in_group - 1],]
        self.player.risk = risks[0]

        for player in self.group.get_players():
            if not player is self.player:
                risks.append(coo_treatment_risks[player.id_in_group - 1])
...
class Bomb(Page):
    ...
    def is_displayed(self):
        return True
        #return (not self.group.adapt) and \
        #       (self.player.adapt==0)
...
page_sequence = [GroupInstr, ChatWait, GroupChoice, VotingWait, SelfInstructions, SelfChoice, Bomb,  Results]
