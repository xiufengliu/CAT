...
(function(angular){
	...
	BombTaskController.prototype._initSettings = function()
		{
			...
			this.must_hit = this._getConstant('MUST_HIT');
			this.resolved = false;
			this.hit_bomb = 0;
	 ...
	 * @ignore
	 */
	BombTaskController.prototype._initBomb = function(bomb)
		{
            var tempArray = [];
            for (var i=0; i<this.rows; ++i){
                for (var j=0; j<this.cols; ++j){
            ...
            if (bomb!=null){
               ...
               var i = 0;
               while (this.bombs.length<this.num_bombs)  {
                    var b = this.matrix[inds[i][0]][inds[i][1]];
                    if (!angular.equals(b, bomb)){
                         this.bombs.push(b);
                    }
                    ++i;
               }
